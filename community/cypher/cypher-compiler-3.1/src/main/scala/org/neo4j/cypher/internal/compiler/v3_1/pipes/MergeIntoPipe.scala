...
import org.neo4j.cypher.internal.compiler.v3_1.mutation.{GraphElementPropertyFunctions, SetAction, makeValueNeoSafe}
...
import org.neo4j.cypher.internal.frontend.v3_1.{InternalException, InvalidSemanticsException, SemanticDirection}
        ...
        if (fromNode == null || toNode == null)
          ...
          if (relationships.isEmpty) {
            val relationship = if (dir == SemanticDirection.INCOMING)
              state.query.createRelationship(toNode.getId, fromNode.getId, typeId, true)
            else
              state.query.createRelationship(fromNode.getId, toNode.getId, typeId, true)
