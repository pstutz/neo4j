 ...
 */
public class CreateVirtualCypherTests {
...
//            r = db.execute("CREATE VIRTUAL (n:Test), (m:Person) " +
//                    " MERGE (n)<-[w:WROTE]-(m) ON CREATE SET n." + CONST.PROPERTYKEY + "=false, " +
//                    "m."+ CONST.PROPERTYKEY + "=false " +
...
//    }
    ...
    public void MergeVirtualSetFalseShouldWorkTransactionWide() {
        Result r;
        try (Transaction tx = db.beginTx()) {
            Result re = db.execute("CREATE VIRTUAL (n:Test), (m:Person) ");
            Assert.assertNotNull("Result should not be null", re);
            r = db.execute("MERGE (n:Test)<-[w:WROTE]-(m:Person) ON MATCH SET n." +
                    CONST.PROPERTYKEY + "=false, " +
                    "m."+ CONST.PROPERTYKEY + "=false " +
                    " RETURN Labels(n), Labels(m), Type(w)");
        ...
        while (r.hasNext()) {
            ...
            result = map.get(sit.next()).toString();
            Assert.assertEquals("[Test]",result);
            result = map.get(sit.next()).toString();
            Assert.assertEquals("WROTE",result);
    ...
    public void virtualSetFalseShouldWorkTransactionWide() {
        Result r;
        try (Transaction tx = db.beginTx()) {
            Result re = db.execute("CREATE VIRTUAL (n:Test)<-[:WROTE]-(m:Person)");
            Assert.assertNotNull("Result should not be null", re);
            r = db.execute("MATCH (n:Test)<-[w:WROTE]-(m:Person) SET n." +
                    CONST.PROPERTYKEY + "=false, " +
                    "m."+ CONST.PROPERTYKEY + "=false " +
                    " RETURN Labels(n), Labels(m), Type(w)");
    ...
    public void complexCreateShouldWork() {
        Result r;
        try (Transaction tx = db.beginTx()) {
            r = db.execute("CREATE (n:Test) " +
                    "WITH n CREATE (m:Person) WITH n,m create " +
                    "(m)-[w:WROTE]->(n) RETURN Labels(n), Labels(m), Type(w)");
            tx.success();
        }
        Assert.assertNotNull("Result should not be null", r);
        while (r.hasNext()) {
    ...
    public void complexVirtualCreateQueryShouldWork() {
        Result r;
        try (Transaction tx = db.beginTx()) {
            r = db.execute("CREATE VIRTUAL (n:Test) " +
                    "WITH n CREATE VIRTUAL (m:Person) WITH n,m create VIRTUAL " +
                    "p = (((m)-[w:WROTE]->(n))) RETURN Labels(n), Labels(m), Type(w)");
            tx.success();
        }
        Assert.assertNotNull("Result should not be null", r);
        while (r.hasNext()) {
    ...
    public void virtualNodesShouldWork() {
        ...
        while (r.hasNext()) {
            ...
            result = map.get(sit.next()).toString();
            Assert.assertEquals("Label of the virtual node should be set to 'TEST'", "[TEST]",
                    result);
        }
        r.close();
        //Clean up
    ...
    public void MergeClauseShouldNotBeAffectedWithoutCreateVirtual(){
        Result r;
        try (Transaction tx = db.beginTx()) {
            r = db.execute(" MERGE (n:Test {Name: 'Hello'}) RETURN " +
                    "n.Name as Name");
            Assert.assertNotNull("Result should not be null", r);
            tx.success();
    ...
    public void ifVirtualEntitiesAreMentionedInMergeTheyShallRemainVirtual(){
        Result r;
        try (Transaction tx = db.beginTx()) {
            r = db.execute(" CREATE VIRTUAL (n:Test {Name: 'Hello'}) " +
            ...
            while (r.hasNext()) {
                ...
                String result = map.get("n.Name").toString();
...
//                String[] split = result.split(",");  // seems that the ordering is fixed
                ...
                Assert.assertEquals("Both entities should be the same, so their ids should match ",
                        map.get("id(n)").toString(), map.get("id(m)").toString()); // everything after id(x)=
                Assert.assertEquals("Both entities should be the same, so their properties should match ",
                        map.get("n.Name").toString(), map.get("m.Name").toString());  // everything after x.Name=
            }
            r.close();
        }
        ...
        try (Transaction tx = db.beginTx()) {
            ...
            // this should return an unable to load node ... error
        }
    ...
    public void virtualNodesShouldLookLikePersistedDuringATransaction() {
        Result r;
        try (Transaction tx = db.beginTx()) {
            Result e = db.execute("CREATE VIRTUAL (n:TEST{name: 'Hello'}) ");
...
//            result = result.replace(" ","");
//            String[] split = result.split(",");
            Assert.assertEquals("Type should be set to 'WROTE'", "WROTE", result);
            result = map.get(sit.next()).toString();
            Assert.assertEquals("Property of the virtual relationship should be set to 'Timestamp=today'", "today",
                    result);
        }
        r.close();
        //Clean up
...
//            String[] split = result.split(",");
    ...
    private Result createRealNode(){
        Result r;
        try (Transaction tx = db.beginTx()) {
            r = db.execute("CREATE (n:TEST{name: 'Hello'"+
